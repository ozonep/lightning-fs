...
module.exports = class NativeFS {
  ...
  async mkdir (dirpath) {
    ...
    const dir = await this._lookupDir(dirname)
    try {
      const newDir = await dir.getDirectoryHandle(basename, { create: true })
  ...
  async rmdir (dirpath, opts = {}) {
    ...
    try {
      ...
      if (recursive) {
        for (const key of this._dirHandles.keys()) {
          if (key.startsWith(dirpath)) this._dirHandles.delete(key)
        }
        for (const key of this._fileHandles.keys()) {
  ...
  async rename (oldFilepath, newFilepath) {
    const [type, h] = await this._lookupUnknown(oldFilepath)
    if (type === 'file') {
      const entry = this.readFile(oldFilepath)
      this.writeFile(newFilepath, entry)
      this.unlink(oldFilepath)
    } else {
      throw new Error(
        'TODO: renaming Directories isnt implemented yet, because this is expensive operation. Waiting for native support in FSA API'
      )
    }
  }
  ...
  async stat (filepath) {
    filepath = path.normalize(filepath)
    const [type, h] = await this._lookupUnknown(filepath)
    return {
      type,
